...
//		HeidelTimeStandalone.main(args);
		...
		String configPath = NerwipFileNames.FO_HEIDELTIME + File.separator + "config.props";
		...
		String fileName = NerwipFileNames.FO_HEIDELTIME+File.separator+"test-fr.txt";
		...
		{	logger.log("Now dealing with specific="+specific);
			...
			{	logger.log("Now dealing with useCategories="+useCats);
				...
				{	logger.log("Now dealing with combineMode="+combineMode);
					...
					SvmCombiner svmCombiner = new SvmCombiner(loadModelOnDemand, specific, useCats, combineMode);
 ...
 * 
 * - use WP instead of FB when retrieving articles (to get the category)
 * - check the retriever works correctly
 * 		- check where the cleaning is done
 */
