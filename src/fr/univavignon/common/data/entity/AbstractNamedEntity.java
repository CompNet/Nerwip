...
{	
	...
	private static final String ATT_KNOWLEDGE_BASE = "knowledgeBase";
	/** Name of the entity */
	private static final String ATT_NAME = "name";
	/** One of the surface forms of an entity */
	private static final String ELT_SURFACE_FORM = "surfaceForm";
	/** List of the surface forms of an entity */
	private static final String ELT_SURFACE_FORMS = "surfaceForms";
	/** One of the external ids (DBpedia, Freebase, etc.) of an entity */
	private static final String ELT_EXTERNAL_ID = "externalId";
	/** List of the external ids (DBpedia, Freebase, etc.) of an entity */
	private static final String ELT_EXTERNAL_IDS = "externalIds";
	
	@Override
	public Element exportAsElement()
	{	Element result = new Element(Entities.ELT_ENTITY);
		...
		// set internal id
		{	Attribute internalIdAttr = new Attribute(ATT_ID, Long.toString(internalId));
			result.setAttribute(internalIdAttr);
		}
		// set main name
		{	Attribute nameAttr = new Attribute(ATT_NAME, name);
			result.setAttribute(nameAttr);
		}
		// set surface forms
		{	Element surfaceFormsElt = new Element(ELT_SURFACE_FORMS);
			result.addContent(surfaceFormsElt);
			for(String surfaceForm: surfaceForms)
			{	Element surfaceFormElt = new Element(ELT_SURFACE_FORM);
				surfaceFormElt.setText(surfaceForm);
				surfaceFormsElt.addContent(surfaceFormElt);
			}
		}
		// external ids
		{	Element externalIdsElt = new Element(ELT_EXTERNAL_IDS);
			...
			{	// retrieve the data
				...
				// set up id element
				Element externalIdElt = new Element(ELT_EXTERNAL_ID);
				externalIdElt.setText(externalId);
				// set up knowledge base attribute
				Attribute kbAttr = new Attribute(ATT_KNOWLEDGE_BASE, kb);
	...
	{	// get the id
		String internalIdStr = element.getAttributeValue(ATT_ID);
		long internalId = Long.parseLong(internalIdStr);
		
		// get the name
		String name = element.getAttributeValue(ATT_NAME);
		
		// build the entity
		AbstractNamedEntity result = AbstractNamedEntity.buildEntity(internalId, name, type);

		// get the surface forms
		{	Element surfaceFormsElt = element.getChild(ELT_SURFACE_FORMS);
		...
		{	Element externalIdsElt = element.getChild(ELT_EXTERNAL_IDS);
			...
			{	// external id
				...
				// knowledge base name
				String kb = element.getAttributeValue(ATT_KNOWLEDGE_BASE);
				// add to result
