...
{	
	...
	private final static String FI_WIKIDATA = "wikidata" + FileNames.EX_TEXT;
	/** File used to cache the mapping queries and their results in a cache */
	private final static String FI_WIKIMEDIA = "wikimedia" + FileNames.EX_TEXT;
	/** File used to cache the mapping queries and their results in a cache */
	private final static String FI_QUERIES = "queries" + FileNames.EX_TEXT;
	...
	{	Set<String> names = entity.getSurfaceForms();
		...
		if(type==EntityType.PERSON)
			possibleNames = CommonStringTools.getPossibleNames(names);
		else
	...
	{	logger.log("Retrieving ids for entity "+possibleNames.get(0)+" (in "+language+")");
		...
		{	String candidateName = it.next();
			...
			{	if(wikidataCache==null)
					wikidataCache = new WmCache(FI_WIKIDATA);
...
//		while(it.hasNext()); //for testing
	...
	{	logger.log("Processing the disambiguation page "+label+" (in "+language+")");
		...
		{	if(wikimediaCache==null)
				wikimediaCache = new WmCache(FI_WIKIMEDIA);
	...
	{	logger.log("Retrieving the types associated to id "+id);
		...
		if(cache)
		{	if(sparqlCache==null)
				sparqlCache = new WmCache(FI_QUERIES);
