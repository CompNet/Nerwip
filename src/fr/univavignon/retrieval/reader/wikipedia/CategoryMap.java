package fr.univavignon.retrieval.reader.wikipedia;
...
import fr.univavignon.retrieval.tools.RetrieverFileNames;
...
{	
	...
	{	// schema file
		...
		{	// load file
			...
			// accept
			Element acceptElt = root.getChild(ELT_ACCEPT);
			if(acceptElt!=null)
				processAcceptElement(acceptElt);
			
			// reject
			Element rejectElt = root.getChild(ELT_REJECT);
			if(rejectElt!=null)
	...
	{	List<Element> catElts = (List<Element>)root.getChildren(ELT_CATEGORY);
		for(Element catElt: catElts)
		{	String catStr = catElt.getAttributeValue(ATT_NAME);
	...
	{	List<Element> keyElts = (List<Element>)root.getChildren(ELT_KEY);
	...
	{	// start expressions
		Element startsWithElt = root.getChild(ELT_STARTS_WITH);
		if(startsWithElt!=null)
			processXxxsWithElement(startsWithElt,true);
			
		// end expressions
		Element endsWithElt = root.getChild(ELT_ENDS_WITH);
		if(endsWithElt!=null)
