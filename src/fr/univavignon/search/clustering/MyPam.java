...
/**
 * Note by Vincent Labatut: there was a bug in the original class,
 * this is just the corrected version.
 * 
 * @author Edward Raff
 */
@SuppressWarnings({ "javadoc", "unchecked", "rawtypes" , "unused" })
public class MyPam extends KClustererBase
{
    ...
    public MyPam(DistanceMetric dm, Random rand, SeedSelection seedSelection)
    {
        this.dm = dm;
        this.rand = rand;
    ...
    public MyPam(DistanceMetric dm, Random rand)
    {
        this(dm, rand, SeedSelection.KPP);
    }

    public MyPam(DistanceMetric dm)
    {
	...
	protected double cluster(DataSet data, boolean doInit, int[] medioids, int[] assignments, List<Double> cacheAccel)
    {
        ...
        if(doInit)
        {
            ...
            selectIntialPoints(data, medioids, dm, cacheAccel, rand, seedSelection);
        }
...
//            this.medioids = medioids;
    ...
    public int[] cluster(DataSet dataSet, int lowK, int highK, ExecutorService threadpool, int[] designations)
    {
        ...
        while(received < totDistances.length)
        {
            ...
            {
                Logger.getLogger(MyPam.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
